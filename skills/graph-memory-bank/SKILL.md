...
2. `project/` (семантика: архитектура, сущности, контракты, API, БД, RF-экраны и т.д.)
3. `processes/` (процессы: правила ведения графа, как обновлять, как читать, качество, генераторы)

Если `processes/` пока не нужен, начинай с первых двух, но держи место под третий слой.
